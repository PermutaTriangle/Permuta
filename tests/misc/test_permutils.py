...
class TestGoodInputsInReverseInverseRotate(unittest.TestCase):
    ...
    def test_rotate_set(self):
        for x in self.inp:
            self.assertEqual(rotate_set(x["input"]), x["rotate"])

    def test_inverse_set(self):
        for x in self.inp:
            self.assertEqual(inverse_set(x["input"]), x["inverse"])

    def test_reverse_set(self):
        for x in self.inp:
            self.assertEqual(reverse_set(x["input"]), x["reverse"])


class TestRoundtripInRotateInverseReverse(unittest.TestCase):
    def test_roundtrip_rotate(self):
        for i in range(100):
...
class TestReturnCharacteristicsInRotateInverseReverse(unittest.TestCase):

    def test_rotate_type(self):
        p = PermSet(10).random()
        self.assertEqual(type(rotate_set(p)), Perm)
        self.assertEqual(type(rotate_set([p])), list)
        self.assertEqual(type(rotate_set((p,))), tuple)
        self.assertEqual(type(rotate_set({p})), set)

    def test_rotate_set_length(self):
        perm_list = [PermSet(6).random() for i in range(10)]
        perm_tup = tuple(perm_list)
        perm_set = set(perm_list)

        self.assertEqual(len(rotate_set(perm_list)), len(perm_list))
        self.assertEqual(len(rotate_set(perm_tup)), len(perm_tup))
        self.assertEqual(len(rotate_set(perm_set)), len(perm_set))

    def test_inverse_type(self):
        p = PermSet(10).random()
        self.assertEqual(type(inverse_set(p)), Perm)
        self.assertEqual(type(inverse_set([p])), list)
        self.assertEqual(type(inverse_set((p,))), tuple)
        self.assertEqual(type(inverse_set({p})), set)

    def test_inverse_set_length(self):
        perm_list = [PermSet(6).random() for i in range(10)]
        perm_tup = tuple(perm_list)
        perm_set = set(perm_list)

        self.assertEqual(len(inverse_set(perm_list)), len(perm_list))
        self.assertEqual(len(inverse_set(perm_tup)), len(perm_tup))
        self.assertEqual(len(inverse_set(perm_set)), len(perm_set))

    def test_reverse_type(self):
        p = PermSet(10).random()
        self.assertEqual(type(reverse_set(p)), Perm)
        self.assertEqual(type(reverse_set([p])), list)
        self.assertEqual(type(reverse_set((p,))), tuple)
        self.assertEqual(type(reverse_set({p})), set)

    def test_reverse_set_length(self):
        perm_list = [PermSet(6).random() for i in range(10)]
        perm_tup = tuple(perm_list)
        perm_set = set(perm_list)

        self.assertEqual(len(reverse_set(perm_list)), len(perm_list))
        self.assertEqual(len(reverse_set(perm_tup)), len(perm_tup))
        self.assertEqual(len(reverse_set(perm_set)), len(perm_set))


class TestBadInputsInRotateInverseReverse(unittest.TestCase):
...
class TestGoodInputsAllSymmetries(unittest.TestCase):
    def test_input_all_symmetries(self):
        inp = [
            {(Perm(()),)},
            {(Perm((0,)),)},
            {(Perm((0, 1)),), (Perm((1, 0)),)},
        ...
        for x in inp:
            for i in x:
                self.assertEqual(all_symmetry_sets(i), x)


class TestBadInputsAllSymmetries(unittest.TestCase):
    def test_all_symmetries_raise_TypeError(self):
        inp = (2,
               2.04,
               1.0,
               0,
